...
async def start(bv, oid, pageID, count, csv_writer, is_second):
    ...
    for reply in comment['data']['replies']:
        ...
        # 写入CSV文件
        csv_writer.writerow(
            [count, parent, rpid, uid, name, level, sex, context, reply_time, rereply, like, sign, IP, vip, avatar])
        ...
        if is_second and rereply != 0:
            for page in range(1, rereply // 10 + 2):
                ...
                for second in second_comment['data']['replies']:
                    ...
                    # 写入CSV文件
                    csv_writer.writerow(
                        [count, parent, second_rpid, uid, name, level, sex, context, reply_time, rereply, like, sign,
                         IP, vip, avatar])
    # 下一页的pageID
    try:
...
async def main(bv: str):
    ...
    with open(file_path, mode='w', newline='', encoding='utf-8-sig') as file:
        ...
        csv_writer.writerow(
            ['序号', '上级评论ID', '评论ID', '用户ID', '用户名', '用户等级', '性别', '评论内容', '评论时间', '回复数',
             '点赞数', '个性签名', 'IP属地', '是否是大会员', '头像'])

        # 开始爬取
        while next_page_id != 0:
            bv, oid, next_page_id, count, csv_writer, is_second = await start(bv, oid, next_page_id, count, csv_writer,
                                                                              is_second)
