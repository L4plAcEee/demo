...
public:
    ...
    Board() : board() {
        board.resize(BOARD_SIZE, std::vector<PlayerType>(BOARD_SIZE, PlayerType::EMPTY));
    ...
    void reset() {
        zobristHash = 0LL;
        for (auto& row : board) {
            std::fill(row.begin(), row.end(), PlayerType::EMPTY);
        }
    }

    bool place(uint64_t x, uint64_t y, PlayerType player) {
        if (x >= BOARD_SIZE || y >= BOARD_SIZE || board[x][y] != PlayerType::EMPTY) {
    ...
    void undo(uint64_t x, uint64_t y, PlayerType player) {
        board[x][y] = PlayerType::EMPTY;
        zobristHash ^= zobristTable[x][y][static_cast<int>(player)];
        currentRound--;
    }

    bool checkWin(uint64_t x, uint64_t y, PlayerType player) const {
